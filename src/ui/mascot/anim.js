...
export function reset(el) {
  ...
  /* Сброс через `!important`, а не через пустую строку.
     После перелётов на элементе остаётся смещение, которое переживает и
     отмену всех анимаций, и очистку инлайнового стиля: ни одного правила,
     ни одной анимации, ни одного инлайнового значения — а вычисленный
     transform всё равно показывает translate. Обычное инлайновое `none`
     такое не перебивает, важное — перебивает. Сидящему в гнезде персонажу
     собственный transform не нужен вовсе, поэтому здесь он прибивается
     насмерть; на время полёта (toLayer) объявление снимается. */
